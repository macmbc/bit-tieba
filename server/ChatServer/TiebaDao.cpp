...
{
	std::cout << "[DEBUG] CreateTieba: Getting connection from pool..." << std::endl;
	auto con = pool_->getConnection();
	std::cout << "[DEBUG] CreateTieba: Got connection, con=" << (con ? "valid" : "null") << std::endl;
	if (con == nullptr)
	{
		std::cout << "[ERROR] CreateTieba: Failed to get connection" << std::endl;
		return -1;
	}
	...
	try
	{
		std::cout << "[DEBUG] CreateTieba: Starting transaction" << std::endl;
		con->_con->setAutoCommit(false);

		// 检查贴吧名称是否已存在
		std::cout << "[DEBUG] CreateTieba: Checking if tieba exists" << std::endl;
...
{
	std::cout << "[DEBUG] GetPostInfo: uid=" << uid << ", post_id=" << post_id << std::endl;
	auto con = pool_->getConnection();
	if (con == nullptr)
	{
		std::cout << "[ERROR] GetPostInfo: Failed to get connection" << std::endl;
		return false;
	}
	...
	try
	{
		std::cout << "[DEBUG] GetPostInfo: Preparing SQL query" << std::endl;
		std::unique_ptr<sql::PreparedStatement> pstmt(con->_con->prepareStatement(
		...
		pstmt->setInt(1, post_id);
		std::cout << "[DEBUG] GetPostInfo: Executing query" << std::endl;
		std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());

		if (res->next())
		{
			std::cout << "[DEBUG] GetPostInfo: Post found" << std::endl;
		...
		std::cout << "[DEBUG] GetPostInfo: Post not found (no rows returned)" << std::endl;
		return false;
	}
	catch (sql::SQLException &e)
	{
