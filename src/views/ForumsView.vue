      ...
      <aside class="filter-panel" aria-label="筛选">
        ...
        <div class="filter-group">
          <div class="group-head">
            <span>筛选标签</span>
            <button v-if="activeFilter !== 'all' || searchKeyword" type="button" class="ghost-btn" @click="resetFilters">
              清除
            </button>
          </div>
        ...
        <div v-else class="forum-grid">
          <article v-for="forum in displayedForums" :key="forum.id" class="forum-card">
            <router-link :to="`/forum/${forum.id}`" class="card-link">
              <div class="card-head">
                <div class="logo" aria-hidden="true">#{{ forum.id }}</div>
                <div>
                  <h3>{{ forum.name }}</h3>
...
.primary-btn {
  ...
  cursor: pointer;
  transition: background-color var(--ease-fast) ease, transform var(--ease-fast) ease;
}
