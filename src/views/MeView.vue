  ...
  <div class="me-page">
    <section class="profile-hero">
      <div class="profile-main">
        <img :src="getUserAvatar(String(getUid()))" alt="头像" class="avatar" />
...
import { getUid, getUsername } from '@/utils/user_info'
...
onMounted(async () => {
  ...
  userInfo.value = {
    username: getUsername(), // 用邮箱前缀作为用户名
    bio: '这个人很懒，什么都没写~',
  ...
  try {
    ...
    posts.value = userPosts.slice(0, 2)
...
const statCards = computed(() => [
  {
    ...
    hint: posts.value.length
      ? `最新帖子收到 ${posts.value[0]?.replyCount ?? 0} 条互动`
      : '快去发布你的第一条帖子',
  },
  {
    label: '关注的吧',
